...
bool test_source_line_info_builder_fuzz_files() {
...
#define SAMPLES_COUNT 0x200
		RzBinSourceFile samples[SAMPLES_COUNT] = { 0 };
		for (size_t i = 0; i < SAMPLES_COUNT; i++) {
			samples[i].address = rand() % 0x100;
			if (rand() % 10 > 2) {
				// non-closing entry
				samples[i].file = (char *)test_filenames[rand() % RZ_ARRAY_SIZE(test_filenames)];
			}
			rz_bin_source_line_info_builder_push_file_sample(&bob, samples[i].address, samples[i].file);
		}
		RzBinSourceLineInfo *li = rz_bin_source_line_info_builder_build_and_fini(&bob);

		// every resulting sample must correspond to one of our original samples
		// also there must be no duplicate addrs and, for optimization, no samples with the same contents after each other
		for (size_t i = 0; i < li->files_count; i++) {
			RzBinSourceFile *f = &li->files[i];
			bool found = false;
			for (size_t j = 0; j < SAMPLES_COUNT; j++) {
				RzBinSourceFile *o = &samples[j];
				if (o->address == f->address &&
					((!o->file && !f->file) || (o->file && f->file && !strcmp(o->file, f->file)))) {
					found = true;
					break;
				}
			}
			mu_assert_true(found, "resulting sample mapped back");

			if (i) {
				RzBinSourceFile *prev = &li->files[i - 1];
				RzBinSourceFile *cur = &li->files[i];
				mu_assert_true(prev->address < cur->address, "strictly increasing addr");
				mu_assert_true(
					(!prev->file != !cur->file) || (prev->file && cur->file && strcmp(prev->file, cur->file)),
					"redundant entry");
			}
		}

		// each of the original samples must be correctly represented, unless it was correctly overwritten
		for (size_t i = 0; i < SAMPLES_COUNT; i++) {
			RzBinSourceFile *o = &samples[i];
			// check if o would be overwritten
			for (size_t j = 0; j < SAMPLES_COUNT; j++) {
				if (j == i) {
					continue;
				}
				RzBinSourceFile *o2 = &samples[j];
				if (o2->address != o->address) {
					continue;
				}
				if ((!o->file && o2->file) || (o->file && o2->file && strcmp(o->file, o2->file) < 0)) {
					o = NULL;
					break;
				}
			}
			...
			const RzBinSourceFile *f = rz_bin_source_line_info_get_file_at(li, o->address);
			if (o->file) {
				mu_assert_notnull(f, "sample");
				mu_assert_true(f->address <= o->address, "sample addr");
				mu_assert_notnull(f->file, "sample file");
				mu_assert_streq(f->file, o->file, "sample file");
			} else {
				mu_assert_null(f, "sample");
			}
		}
...
bool all_tests() {
	...
	mu_run_test(test_source_line_info_builder_fuzz_files);
